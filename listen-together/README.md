...
Each IP (each /64 for IPv6) may hold at most 8 connections and four rooms; the whole relay holds at most 200 connections. Both caps can be changed with `MAX_CONNECTIONS_PER_IP` and `MAX_CLIENTS`. Rooms with only their host expire after 30 minutes; active rooms retain the six-hour maximum. WebSocket pong grace is 30 seconds to tolerate brief network interruptions. Host loss still ends a prototype room; invitations do not reconnect a host automatically.

Browser origins must match the relay host or be listed in comma-separated `ALLOWED_ORIGINS`. Desktop file origins (`null`) and clients without an Origin header are accepted. Any web page can also send `null` from a sandboxed frame, so the allowlist is advisory: it stops ordinary cross-site pages, not a determined one. Invitations, host-only publishing and the per-IP caps are what protect rooms. `TRUST_PROXY=1` trusts the last X-Forwarded-For hop only when the TCP peer is loopback. Set it only behind a loopback TLS proxy that overwrites/appends the client address; forwarded headers are otherwise ignored. If the proxy does not send the header, every client shares the proxy's limits, and the relay logs a warning.
...
Guests now retry a dropped connection up to three times (1/2/4 second backoff) while the host room remains available. Playback pauses during reconnect and resumes from the latest host snapshot. Explicit leave, protocol errors and room-ended messages do not reconnect. Host connection loss still ends the room; host migration is not implemented. The allowlisted snapshot includes whether the selected track is a video, so guests can display that same version without changing the host's selection.
