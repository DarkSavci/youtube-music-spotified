...
Playback review notes: advancing to a song does not automatically replace its ID with a video counterpart, so audio prefetch/gapless playback remains intact. Select Video explicitly for that song. Picture synchronization waits through buffering/seeking and uses small playback-rate corrections before a hard seek. Moving the shared video element between documents can make Chromium reload picture data; the audio engine remains independent and continues playing.
