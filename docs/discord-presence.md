...
Live macOS verification (2026-09-25): a user-owned test application completed the local IPC handshake; Discord acknowledged SET_ACTIVITY, displayed “Listening to Youtube Music Spotified Test” on the user profile, and acknowledged clearing activity on pause. This verifies the local integration and listening type. A second-account check of artwork/buttons and Windows live verification remain outstanding. The test application ID is not shipped as a default.
